...
for post in listdir('app/content/posts'):
    filename = os.path.splitext(post)[0]
    if filename != '.DS_Store':  # is there a better way to filter this out?
        print(filename)
        with open('app/content/posts/' + filename + '.toml') as conffile:   #app.listen load all posts in memory and save in object and reference dictionary
